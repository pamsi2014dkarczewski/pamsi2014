...
public:
  ...
  {
    wartosc = wart; /* do 'wartosc' przypisywana jest wartosc ktora zostanie dodana na stos */
...
public:
  ...
  void add_front(Typ dana); /* dodanie elementu na poczatek listy */
  Typ *delete_front(); /* usuniecie elementu z poczatku listy i zwrocenie zapisanej wartosci */
...
/****************************** DODANIE NA POCZATEK **************************/
template <typename Typ>
void stos<Typ>::add_front(Typ dana)
{
  /* stworzenie nowego elementu na poczatku listy z wartoscia 'dana' ktory bedzie pokazywal na nastepny element */
  ...
  if(!stos_pusty()) /* jezeli stos nie jest pusty */
    {
      ...
      element_listy<Typ> *temp = poczatek; /* pomocniczy wskaznik wskazujacy na poczatek listy */
      ...
      else /* jezeli lista zawiera kilka elementow */
	{
      ...
      rozmiar_stosu--; /* pomniejszenie rozmiaru stosu, poniewaz element zostal usuniety */
      return element;
    }
